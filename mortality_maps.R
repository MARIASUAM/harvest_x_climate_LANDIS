...
mgmt.scenarios <- c(
                    ...
                    # "211129_proactiveplus_rcp85_rep2"
                    ) # Folder names with each replicate
...
spp <- c("ppinaster", "pnigra", "phalepensis", "psylvestris", "qilex", "qfaginea", "qpyrenaica") 
...
for (h in seq_along(mgmt.scenarios)) {
  for (k in seq_along(spp)) {
    for (j in 2:length(timesteps)) {
      ...
      # diff_coh <- nr_coh_without_first_t2 - nr_coh_without_first_t1 # Previous calculation, mistaken!?
      diff_coh <- nr_coh_t1 - nr_coh_without_first_t2

      # Mask by pine plantations 
      diff_coh <- mask(diff_coh, pines_mask)
      
      # Export
      writeRaster(diff_coh, paste(di, mgmt.scenarios[h], "/results/", spp[k], "_diff_cohorts_", timesteps[j], "_revised.asc", sep = ""), overwrite = TRUE)
    }
  }
}
