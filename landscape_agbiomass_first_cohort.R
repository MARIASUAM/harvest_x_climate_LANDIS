...
for (i in seq_along(mgmt.scenarios)) {
  for (j in seq_along(replicates)) {
    one_replicate <- read.table(paste(di, mgmt.scenarios[i], "_rep", replicates[j], "/results/aggregated_agbiomass_by_age_dense_pines_oaks_pines.txt", sep =""), header = TRUE) %>% 
...
agb_dense_pines_spp$Harv_scenario <- factor(agb_dense_pines_spp$Harv_scenario, 
                                        levels=c("No Management", "Conservative", "Proactive", "ProactivePlus"))
...
ggplot(agb_dense_pines_spp, aes(x = Year, y = Mean_group_avg_tnha_across_reps)) +
  ...
  theme(legend.position = "bottom",
        ...
        legend.text = element_text(size = 14),
        legend.title = element_blank()) +
  ylab("Average biomass (tn/ha) of youngest cohort") +
