---
title: "Compare observations and modelled data - MINIMUM TEMPERATURE"
...
# Load observations
obs <- read.table(paste(di, "calculated_month_tmin_TD2.txt", sep = ""), header = TRUE, sep = ";") %>%
...
my_stack <- stack(paste(folder_harddisk, "tasmin_ACCESS1-3_historical_1950_2005.nc", sep = ""), varname = "air_temperature")
...
colnames(ACCESS_modelled) <- c("Layer", "INDICATIVO", "ACCESS_month_tmin_K")

## CESM model
# Load stack from model
my_stack <- stack(paste(folder_harddisk, "tasmin_CESM1-BGC_historical_1950_2005.nc", sep = ""), varname = "air_temperature")
# Extract modelled values 
my_values <- as.matrix(t(raster::extract(my_stack, my_stations_23030_spdf, method = 'simple'))) 
...
colnames(CESM_modelled) <- c("Layer", "INDICATIVO", "CESM_month_tmin_K")

## CMCC model
# Load stack from model
my_stack <- stack(paste(folder_harddisk, "tasmin_CMCC-CM_historical_1950_2005.nc", sep = ""), varname = "air_temperature")
# Extract modelled values 
my_values <- as.matrix(t(raster::extract(my_stack, my_stations_23030_spdf, method = 'simple'))) 
...
colnames(CMCC_modelled) <- c("Layer", "INDICATIVO", "CMCC_month_tmin_K")

## CMCC model
# Load stack from model
my_stack <- stack(paste(folder_harddisk, "tasmin_MIROC5_historical_1950_2005.nc", sep = ""), varname = "air_temperature")
# Extract modelled values 
my_values <- as.matrix(t(raster::extract(my_stack, my_stations_23030_spdf, method = 'simple'))) 
...
colnames(MIROC_modelled) <- c("Layer", "INDICATIVO", "MIROC_month_tmin_K")
...
obs_modelled <- obs %>%
  ...
  mutate(observed = month_tmin, 
         ACCESS = ACCESS_month_tmin_K - 273.15,
         CESM1 = CESM_month_tmin_K - 273.15,
         CMCC = CMCC_month_tmin_K - 273.15,
         MIROC5 = MIROC_month_tmin_K - 273.15)
...
```{r, echo = FALSE}
jpeg(file = "/Users/maria.suarez.munoz/Google Drive/proj_LANDIS/experiments/outputs/tasmin_comparison.jpeg", width=6, height=6, units="in", res=300)
...
summary_table <- data.frame(Model = c("ACCESS", "CESM1", "CMCC", "MIROC5"),
                            R2 = c(summary(ACCESS_m)$r.squared, summary(CESM1_m)$r.squared, summary(CMCC_m)$r.squared, summary(MIROC5_m)$r.squared),
                            RMSE = c(sqrt(mean((subset_ACCESS$Observed - subset_ACCESS$Modelled)^2)), sqrt(mean((subset_CESM1$Observed - subset_CESM1$Modelled)^2)), sqrt(mean((subset_CMCC$Observed - subset_CMCC$Modelled)^2)), sqrt(mean((subset_MIROC5$Observed - subset_MIROC5$Modelled)^2))))
